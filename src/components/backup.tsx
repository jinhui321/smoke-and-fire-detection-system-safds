...
import { Upload, Camera, CameraOff, FileImage, FileVideo, Play, AlertTriangle, CheckCircle, X, Download } from 'lucide-react';
...
const DetectionTab: React.FC = () => {
  ...
  // New state to preserve final frame and detections
  const [finalProcessedFrame, setFinalProcessedFrame] = useState<string | null>(null);
  const [finalDetections, setFinalDetections] = useState<ProcessedFrame['detections']>([]);
  ...
  const startVideoProcessing = async (videoPath: string) => {
    try {
      ...
      if (response.ok) {
        ...
        // Add a timeout to ensure processing completes (fallback)
        setTimeout(() => {
          if (isVideoProcessing && !annotatedVideoPath) {
            console.log('Video processing timeout - forcing completion');
            setIsVideoProcessing(false);
            // Try to get the video path from the backend
            fetch('http://localhost:8000/stop_video_processing', { method: 'POST' })
              .then(res => res.json())
              .then(result => {
      ...
      try {
        ...
        if (data.completed) {
          ...
          if (data.annotated_video_path) {
            setAnnotatedVideoPath(data.annotated_video_path);
          }
    ...
    <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
      ...
      <div className="bg-gray-900 p-6 rounded-lg min-h-[400px] flex flex-col items-center justify-center">
        {cameraActive ? (
          ...
          <div className="space-y-4 text-center">
            {processedFrame ? (
              <>
                ...
                {/* Auto-complete processing after a delay */}
                <div className="mt-4">
                  <div className="w-12 h-12 border-4 border-blue-500 border-t-transparent rounded-full animate-spin mx-auto mb-2"></div>
                  <p className="text-sm text-gray-400">Processing video frames...</p>
                  <p className="text-xs text-gray-500 mt-1">Processing will complete automatically when the video ends</p>
                </div>
              </>
            ) : (
              <div className="space-y-4">
          ...
          // Video Processing Complete with Final Frame and Download
          <div className="space-y-4 text-center">
            {/* Show final processed frame if available */}
            {finalProcessedFrame && (
              <>
                <img
                  src={finalProcessedFrame}
                  alt="Final Processed Video Frame"
                  className="mx-auto rounded-lg border border-gray-700 max-h-64 object-contain bg-black"
                  style={{ width: "100%", maxWidth: 400 }}
                />
                <div className="text-white">
                  <p className="font-medium">Final Detection Results</p>
                  {finalDetections.length > 0 ? (
                    <div className="mt-2">
                      <p className="text-sm text-gray-400">Final Detections:</p>
                      <div className="flex flex-wrap gap-2 justify-center mt-1">
                        {finalDetections.map((detection, index) => (
                          <span
                            key={index}
                            className={`px-2 py-1 rounded text-xs font-medium ${
                              detection.class.toLowerCase().includes('fire') 
                    ...
                    <p className="text-sm text-gray-400">No detections in final frame</p>
                  )}
                </div>
              </>
            )}
